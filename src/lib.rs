...
// Export the run function for both native and wasm
pub fn run() {
    run_app();
}
...
impl App {
    ...
    fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        // CRITICAL: Ensure we never pass 0 to WebGPU to prevent crashes
        self.size = new_size;
    ...
    fn save_config(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            log::info!("Config save is not supported on web platform");
            Ok(())
        }
    }
    ...
    fn load_config(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            log::info!("Config load is not supported on web platform");
        }
        Ok(())
    }
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
                    ...
                    .show(ctx, |ui| {
                        ...
                        if !self.graph_data.time.is_empty() {
                            ...
                            Plot::new("displacement_plot")
                                ...
                                .show(ui, |plot_ui| {
                                    plot_ui.line(
                                        Line::new("X Position (m)", displacement_x_points)
                                            .name("X Position (m)")
                                            .color(egui::Color32::from_rgb(255, 165, 0)), // Orange for X
                                    );
                                    plot_ui.line(
                                        Line::new("Y Position (m)", displacement_y_points)
                                            .name("Y Position (m)")
                            ...
                            Plot::new("velocity_plot")
                                ...
                                .show(ui, |plot_ui| {
                                    plot_ui.line(
                                        Line::new("X Velocity (m/s)", velocity_x_points)
                                            .name("X Velocity (m/s)")
                                            .color(egui::Color32::from_rgb(255, 165, 0)), // Orange for X
                                    );
                                    plot_ui.line(
                                        Line::new("Y Velocity (m/s)", velocity_y_points)
                                            .name("Y Velocity (m/s)")
                                            .color(egui::Color32::from_rgb(0, 255, 255)), // Cyan for Y
                                    );
                                });
                            ...
                            Plot::new("acceleration_plot")
                                ...
                                .show(ui, |plot_ui| {
                                    plot_ui.line(
                                        Line::new("X Acceleration (m/s²)", acceleration_x_points)
                                            .name("X Acceleration (m/s²)")
                                            .color(egui::Color32::from_rgb(255, 165, 0)), // Orange for X
                                    );
                                    plot_ui.line(
                                        Line::new("Y Acceleration (m/s²)", acceleration_y_points)
        ...
        // Extend the lifetime of the render pass to 'static to satisfy egui_wgpu
        // SAFETY: The render pass is dropped immediately after use, so it won't outlive the encoder.
...
impl ApplicationHandler for PhysicsApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                let app = pollster::block_on(App::new(window_ref));
                self.app = Some(app);
            }
            ...
            #[cfg(target_arch = "wasm32")]
            {
                ...
                wasm_bindgen_futures::spawn_local(async move {
                    let app = App::new(window_ref).await;
                    *app_rc.borrow_mut() = Some(app);
                });
            }
        }
    }
    ...
    fn window_event(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        let app_opt = self.app.as_mut();

        #[cfg(target_arch = "wasm32")]
        let mut app_guard = self.app.borrow_mut();
        #[cfg(target_arch = "wasm32")]
        let app_opt = app_guard.as_mut();
    ...
    fn about_to_wait(&mut self, _event_loop: &ActiveEventLoop) {
        #[cfg(not(target_arch = "wasm32"))]
        let app_opt = self.app.as_ref();

        #[cfg(target_arch = "wasm32")]
        let app_guard = self.app.borrow();
        #[cfg(target_arch = "wasm32")]
        let app_opt = app_guard.as_ref();
...
fn run_app() {
    ...
    let mut physics_app = PhysicsApp {
        window: None,
        #[cfg(not(target_arch = "wasm32"))]
        app: None,
        #[cfg(target_arch = "wasm32")]
